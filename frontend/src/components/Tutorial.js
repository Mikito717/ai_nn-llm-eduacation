  ...
  const runKNN = async () => {
    ...
    if (!isClicked) {
      ...
      try {
        ...
        const response = await fetch('http://localhost:5000/run_knn', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
          // リクエストボディにKの値と距離の計量法を含める
          body: JSON.stringify({ k: kValue, distance_metric: distanceMetric }),
        });
  ...
  return (
    <div className="container">
      ...
      <div className="form-group">
        ...
        <input type="number" id="k-value" min="100" max="10000" defaultValue="100" />
      </div>
      <div className="form-group">
      ...
      <div className="buttons">
        <button id="run-button" onClick={runKNN}>実行</button>
