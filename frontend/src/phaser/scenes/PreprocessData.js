...
class PreprocessData extends Phaser.Scene {
  ...
  create() {
    ...
    this.add.text(100, 200, 'Select how to handle missing values', {
      fill: '#0f0',
    })
    this.hadle1button = this.add.text(
      100,
      250,
      '1. Drop rows with missing values',
      { fill: '#0f0' },
    )
    this.hadle2button = this.add.text(
      100,
      300,
      '2. Fill missing values with mean',
      { fill: '#0f0' },
    )
    this.hadle3button = this.add.text(
      100,
      350,
      '3. Fill missing values with median',
      { fill: '#0f0' },
    )
    this.hadle4button = this.add.text(
      100,
      400,
      '4. Fill missing values with mode',
      { fill: '#0f0' },
    )
    this.hadle5button = this.add.text(
      100,
      450,
      '5. Fill missing values with constant',
      { fill: '#0f0' },
    )
    ...
    this.hadle1button.on('pointerdown', this.prepeocess1, this)
    this.hadle2button.on('pointerdown', this.prepeocess2, this)
    this.hadle3button.on('pointerdown', this.prepeocess3, this)
    this.hadle4button.on('pointerdown', this.prepeocess4, this)
    this.hadle5button.on('pointerdown', this.prepeocess5, this)
